...
import wang.tinycoder.mqtt.MqttClient;
import wang.tinycoder.mqtt.MqttClientListener;
import wang.tinycoder.mqtt.MqttCode;
...
public class DeviceDetailActivity extends BaseActivity<DeviceDetailPresenter> implements DeviceDetailContract.View {
    ...
    private MqttClientListener mqttClientListener = new MqttClientListener() {

        @Override
        public void onSuccessMQTT(MqttCode code, String topic, String message) {
            Logger.i("%s onSuccessMQTT");
            if (MqttCode.CONNECTED_SUCCEED == code) {   // 连接成功，订阅
                mMqttClient.subTopic(gensubscribeToTopic(mDevice.getKey()));
            }
        }

        @Override
        public void onFailureMQTT(MqttCode code, String errorMesage) {
            Logger.e("%s onFailureMQTT , errorMesage ---> %s", TAG, errorMesage);
        }


        @Override
        public void MessageArrived(String topic, MqttMessage message) {
            Logger.i("%s MessageArrived : topic ---> %s , message ---> %s", TAG, topic, new String(message.getPayload()));
        }
    };
    ...
    public void initPresenter() {
        ...
        mDevice = (Device) getIntent().getSerializableExtra(Constants.EXTRA_DEVICE);
    }
    ...
    private void initMqtt() {
        if (mMqttClient == null) {
            mMqttClient = new MqttClient.Builder(this)
                    .setServerUri(Constants.MQTT_URL)
                    .setClientID(mDevice.getId())
                    .setPort(Constants.MQTT_PORT)
                    .setUserName(mDevice.getId())
                    .setPassword(mDevice.getId())
                    .setReConnect(true)
                    .creat();
            mMqttClient.setClientListener(mqttClientListener);
            mMqttClient.startConnect();
        }
    }
     ...
     * 生成订阅消息的topic
     *
     * @param key userid-groupid-deviceid
     * @return
     */
    private String gensubscribeToTopic(String key) {
        ...
        try {
            ...
            return String.format(Constants.RECIVE_TOPIC_TEMP, split[0], split[1], split[2]);
        } catch (Exception e) {
            e.printStackTrace();
        }
